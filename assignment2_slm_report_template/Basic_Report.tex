...
\textbf{Retrieval method: cosine similarity with scikit-learn.} We use scikit-learn's \texttt{cosine\_similarity} to compare the query vector with all chunk vectors. Since we only have about 73 chunks, this is very fast and we do not need heavy tools like FAISS or Chroma. Keeping it simple also makes the code easier to understand and debug.
...
\begin{itemize}
    ...
    \item The retrieval is just brute-force cosine similarity. This is fine for our 73 chunks, but it would be slow if the dataset was much bigger.
\end{itemize}
...
\textit{Note: These scores are our estimates based on reading through the outputs carefully. The full evaluation table with all scores is in Appendix~\ref{app:evaluation}.}
